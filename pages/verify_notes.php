...
if ($db->num_rows($result_check) > 0) {
    echo "<p style='color: green;'>OK: Column 'note' exists in 'products' table.</p>";
} else {
    echo "<p style='color: red;'>ERROR: Column 'note' DOES NOT exist in 'products' table. Please run the database update.</p>";
}
...
if (!empty($products)) {
    ...
    if (array_key_exists('note', $first_product)) {
        echo "<p style='color: green;'>OK: 'join_product_table' returns the 'note' field.</p>";
        echo "<p>Example note value: '" . ($first_product['note'] ?? 'NULL') . "'</p>";
    } else {
        echo "<p style='color: red;'>ERROR: 'join_product_table' DOES NOT return the 'note' field. Please ensure you uploaded the updated 'includes/sql.php'.</p>";
    }
} else {
