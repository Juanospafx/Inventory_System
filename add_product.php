...
if (isset($_POST['add_product'])) {
  ...
  if (empty($errors)) {
    ...
    if ($db->query($query)) {
      ...
      $qr_path = $qr_dir . 'qrcode-' . $product_id . '.png';
      file_put_contents($qr_path, $qr_image);

      $update_query = "UPDATE products SET qr_code='{$db->escape($qr_path)}' WHERE id='{$product_id}'";
      ...
      $session->msg('s', "Producto agregado exitosamente. El código QR ha sido generado.");
